...
(function () {
  ...
  function mount(bodyEl, barEl) {
    ...
    /* 加一格進來，順便判斷手指是「直直劃」還是「轉彎」還是「斜著拉」。
       轉彎＝改變方向，不是要框一個方塊 —— 所以從轉彎處重新起一條線，
       不然往上再往右會多補一個你根本沒碰到的角。 */
    function sample(x, y, own) {
      ...
      var last = S.touch[S.touch.length - 1];
      var dr = Math.floor(p / S.W) - Math.floor(last / S.W);
      var dc = (p % S.W) - (last % S.W);

      if (dr !== 0 && dc !== 0) {
        S.axis = 'box';                   /* 斜著拉：本來就是要框一塊長方形 */
      } else if (S.axis !== 'box') {
        var dir = dc !== 0 ? 'h' : 'v';
        if (S.axis && S.axis !== dir) S.touch = [last];   /* 轉彎：從這裡重新起一條 */
        S.axis = dir;
      }
    ...
    function onMove(e) {
      ...
      if (!added) return;
      /* 斜著拉是「從起點框到現在這格」；直直劃就只算劃過的那一條 */
      S.drag = S.axis === 'box'
        ? boxOf([S.dragFrom, S.touch[S.touch.length - 1]])
        : boxOf(S.touch);
      paintDrag();
    }
  ...
  App.register({
    ...
    help:
      ...
      '<div class="hstep"><span class="hnum">3</span><div class="hbody">' +
        ...
        '<div class="hnote"><u>直直劃</u>一排就是那一排，中途<u>轉彎</u>就從轉彎處重新算，' +
        '不會多圈你沒碰到的格子。<br>要圈成方形的那種，<u>斜著拉</u>到對角就好。' +
        '<br>已經圈好的格子會自動跳過。' +
